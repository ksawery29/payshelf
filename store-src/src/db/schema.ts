...
export const shopSettings = sqliteTable('shop_settings', {
  ...
  waitlistEmailHtml: text('waitlist_email_html'),
  updatedAt: integer('updated_at', { mode: 'timestamp_ms' })
    .default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`)
    .$onUpdate(() => /* @__PURE__ */ new Date())
    .notNull(),
...
export const analyticsEvent = sqliteTable(
  'analytics_event',
  {
    id: text('id')
      ...
      .$defaultFn(() => crypto.randomUUID()),
    event: text('event', {
      enum: ['page_view', 'checkout_initiated', 'checkout_completed', 'checkout_cancelled'],
    }).notNull(),
...
export const cancelFeedback = sqliteTable(
  'cancel_feedback',
  {
    ...
    reason: text('reason', {
      enum: ['too_expensive', 'just_browsing', 'found_alternative', 'trust', 'other'],
    }).notNull(),
