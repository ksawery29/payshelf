...
export const getEventAnalyticsFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    for (const row of trendRows) {
      const entry = dayMap.get(row.date) ?? { page_view: 0, checkout_initiated: 0 }
    ...
    const trendData: { date: string; page_view: number; checkout_initiated: number }[] = []
    for (let i = 0; i < daysBack; i++) {
      const d = new Date(trendCutoff)
      d.setDate(trendCutoff.getDate() + i)
      const dateStr = d.toISOString().split('T')[0]
      const entry = dayMap.get(dateStr) ?? { page_view: 0, checkout_initiated: 0 }
    ...
    const products = await db
      .select({ id: product.id, name: product.name, priceCents: product.priceCents })
      .from(product)
    ...
    const productEventRows = await db
      ...
      .where(
        and(
          gte(analyticsEvent.createdAt, cutoff),
          sql`${analyticsEvent.productId} is not null`,
        ),
      )
